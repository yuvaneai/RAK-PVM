...
def main():
    metadata_df = pd.read_csv(os.path.join(BASE_DIR, "./MIMIC_data/mimic-cxr-2.0.0-metadata.csv"))
    ...
    text_df = pd.read_csv(os.path.join(BASE_DIR, "./MIMIC_data/mimic_cxr_sectioned.csv"))
    ...
    split_df = pd.read_csv(os.path.join(BASE_DIR, "./MIMIC_data/mimic-cxr-2.0.0-split.csv"))
    ...
    chexpert_df = pd.read_csv(os.path.join(BASE_DIR, "./MIMIC_data/mimic-cxr-2.0.0-chexpert.csv"))
    ...
    root_dir = str(os.path.join(BASE_DIR, "./MIMIC_data/mimic-cxr-jpg"))
    path_list = []
    for i in range(n):
        row = master_data[i]
    ...
    train_df.to_csv(os.path.join(BASE_DIR, "./MIMIC_data/0_train.csv"), index=False)
    valid_df = labeled_data_df.loc[labeled_data_df["split"] == "valid"]
    valid_df.to_csv(os.path.join(BASE_DIR, "./MIMIC_data/0_test.csv"), index=False)

    master_df[["impression"]] = master_df[["impression"]].fillna(" ")
    master_df[["findings"]] = master_df[["findings"]].fillna(" ")
    master_df.to_csv(os.path.join(BASE_DIR, "./MIMIC_data/0_master.csv"), index=False)
