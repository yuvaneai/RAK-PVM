...
def make_scheduler(
    ...
    warmup = train_params['params']['warmup_epoch']
    total_iters = train_params['params']['total_epoch']

    if train_params['sch_name'] == "cosine":
        scheduler = WarmupCosineSchedule(
            optimizer,
            warmup_steps=warmup,
            t_total=total_iters
        )
    elif train_params['sch_name'] == "cosine_hardrestart":
        scheduler = WarmupCosineWithHardRestartsSchedule(
            optimizer,
            warmup_steps=warmup,
            t_total=total_iters
        )
    elif train_params['sch_name'] == "linear":
        scheduler = WarmupConstantSchedule(
            optimizer,
            warmup_steps=warmup,
        )
    elif train_params['sch_name'] == "plateau":
