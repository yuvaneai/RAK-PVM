...
report = """
No evidence of intrathoracic metastatic disease. The cardiomediastinal silhouette is normal. 
The lungs are hyperinflated and grossly clear. There is no pneumothorax or pleural effusion. 
Moderate degenerative changes are seen in the thoracic spine unchanged from prior.
"""


predictions = rg(report)
radgraph_output = predictions['0']['entities']
...
def collapse_to_dict_cell(df):
    ...
    df_one = pd.DataFrame({"Triplet Extraction": [str(result_dict)]})
    return df_one

df = parse_radgraph_output(radgraph_output)
df_filtered = filter_entities(df)
df_triplet = collapse_to_dict_cell(df_filtered)

input_file = "E:/3_CL_coding/mimic_data/splits/mimic_part_0.csv"
output_file = "E:/3_CL_coding/mimic_data/splits/mimic_part_0_triplet_1.csv"
