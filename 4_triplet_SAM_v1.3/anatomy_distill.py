...
class SAM_PROMPT(LightningModule):
    def __init__(self,
        ...
        self.anatomy_box_S = AnatomyStudent(
            ...
            hidden_dim=128,
            num_queries=40 # 1 10 20 30 40 50 60 70
            # 必须 ≥ 教师的解剖位置数  学生 num_queries 只要 ≥ 教师一张图最多出现的解剖位置数量，就永远没问题！
        )
        ...
        # 512
        # our_state_dict = torch.load(
        #     "/home/by/by/4_SAM_prompt/4_triplet_SAM_v1.3/result/pretrain_loss_ita_512token/ckpts/2026_05_06_14_05_22/epoch=40-step=18737.ckpt",
        #     map_location=torch.device("cpu"))
        # 768
        # our_state_dict = torch.load(
        #     "/home/by/by/4_SAM_prompt/4_triplet_SAM_v1.3/result/pretrain_loss_ita_768token/ckpts/2026_05_07_01_29_35/epoch=27-step=12796.ckpt",
        #     map_location=torch.device("cpu"))
        # 64
        # our_state_dict = torch.load(
        #     "/home/by/by/4_SAM_prompt/4_triplet_SAM_v1.3/result/pretrain_loss_ita_64token/ckpts/2026_05_07_14_51_54/epoch=35-step=16452.ckpt",
        #     map_location=torch.device("cpu"))
    ...
    def forward(self, batch, batch_idx, split="train"):
        ...
        image_embeds = self.prompt_learner.img_proj(image_embeds)  # 投影
...
def cli_main():
    ...
    ckpt_dir = os.path.join(BASE_DIR, f"result/anatomy_distill_256token_studim128_symptom40/ckpts/{extension}")  # loss_ita+loss_proto
    os.makedirs(ckpt_dir, exist_ok=True)

    # TensorBoard 日志目录
    logger_dir = os.path.join(BASE_DIR, f"result/anatomy_distill_256token_studim128_symptom40/log")  # loss_ita+loss_proto
