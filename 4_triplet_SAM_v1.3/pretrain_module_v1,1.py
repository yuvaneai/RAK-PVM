...
class SAM_PROMPT(LightningModule):
    ...
    def forward(self, batch, batch_idx, split="train"):
        ...
        # OT-based Alignment 🔥🔥
        # =========================
        patch_feat_q = F.normalize(patch_feat_q, dim=-1)
        word_feat = F.normalize(impression_word_feat, dim=-1)

        sim = torch.einsum("bpd,bnd->bpn", patch_feat_q, word_feat)  # [B, P, N]

        # 转成 cost（越小越好）
        cost = -sim

        # soft assignment（近似 OT）
        attn = F.softmax(-cost / 0.05, dim=-1)  # 行归一

        # optional: 列归一（近似双随机）
        attn = attn / (attn.sum(dim=1, keepdim=True) + 1e-6)

        text_agg = torch.einsum("bpn,bnd->bpd", attn, word_feat)

        loss_local = 1 - F.cosine_similarity(patch_feat_q, text_agg, dim=-1).mean()

        return loss_ita, loss_local, teacher_loss, loss_dense_distill


    def training_step(self, batch, batch_idx):
        loss_ita, loss_local, teacher_loss, loss_dense_distill = self(batch, batch_idx, "train")
        loss =  self.hparams.lambda_1 * loss_ita + self.hparams.lambda_2 * teacher_loss + self.hparams.lambda_3 * loss_dense_distill + self.hparams.lambda_3 * loss_local
        self.log_dict({"train_loss": loss},
                      sync_dist=True, prog_bar=True, batch_size=self.hparams.batch_size)
        return loss

    def validation_step(self, batch, batch_idx):
        loss_ita, loss_local, teacher_loss, loss_dense_distill = self(batch, batch_idx, "valid")
        loss =  self.hparams.lambda_1 * loss_ita + self.hparams.lambda_2 * teacher_loss + self.hparams.lambda_3 * loss_dense_distill + self.hparams.lambda_3 * loss_local
...
def cli_main():
    ...
    ckpt_dir = os.path.join(BASE_DIR, f"result/pretrain_loss_ita_local_256token/ckpts/{extension}")  # loss_ita+loss_proto
    os.makedirs(ckpt_dir, exist_ok=True)

    # TensorBoard 日志目录
    logger_dir = os.path.join(BASE_DIR, f"result/pretrain_loss_ita_local_256token/log")  # loss_ita+loss_proto
