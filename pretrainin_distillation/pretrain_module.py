...
class SAM_PROMPT(LightningModule):
    ...
    def forward(self, batch, batch_idx, split="train"):
        ...
        return loss_ita, loss_dense_distill


    def training_step(self, batch, batch_idx):
        loss_ita, loss_dense_distill = self(batch, batch_idx, "train")
        loss =  self.hparams.lambda_1 * loss_ita + self.hparams.lambda_2 * loss_dense_distill
        self.log_dict({"train_loss": loss},
                      sync_dist=True, prog_bar=True, batch_size=self.hparams.batch_size)
        return loss

    def validation_step(self, batch, batch_idx):
        loss_ita, loss_dense_distill = self(batch, batch_idx, "valid")
        loss =  self.hparams.lambda_1 * loss_ita + self.hparams.lambda_2 * loss_dense_distill
...
def cli_main():
    ...
    ckpt_dir = os.path.join(BASE_DIR, f"./result/pretrain/ckpts/{extension}")  # loss_ita+loss_proto
    os.makedirs(ckpt_dir, exist_ok=True)

    logger_dir = os.path.join(BASE_DIR, f"./result/pretrain/result/log")  # loss_ita+loss_proto
